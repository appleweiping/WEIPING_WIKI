...
## Local Codex/D-Drive Override

This installation lives inside `D:/Research/vipin's knowledgebase/.codex/skills/chrome-automation/`, not `~/.claude/skills/`.

For this machine, prefer the already verified D-drive runtime before using the upstream `$HOME\Documents` paths:
...
Verified smoke test on 2026-05-17:

```powershell
Start-Process -FilePath "C:\Program Files\Google\Chrome\Application\chrome.exe" `
  -ArgumentList @("--remote-debugging-port=$PORT", "--user-data-dir=$CHROME_PROFILE", "--no-first-run", "--no-default-browser-check", "about:blank")
...
& $AB_BIN screenshot body "D:\Research\vipin's knowledgebase\.wiki-tmp\chrome-automation-example.png" --cdp $PORT --session codex-smoke
```

The D-drive `agent-browser-win32-x64.exe` returned `agent-browser 0.27.0`, connected to Chrome via CDP, read `Example Domain`, read `https://example.com/`, listed `Example Domain` / `Learn more` refs, and wrote screenshots. On this Windows binary, prefer `agent-browser <command> ... --cdp <port> --session <name>`; putting `--cdp` before the command can reconnect to stale/default state. For saved screenshots, use `screenshot body <path>` or `screenshot --full <path>`; a single path argument can be interpreted as a selector. Native Rust rebuild failed only because MSVC `link.exe` was absent; the downloaded upstream Windows binary works.

## ‚ö†Ô∏è CRITICAL: Automatic Installation & Dependency Check
...
**Critical rules:**
- ‚ù?DO NOT fall back to using built-in browser tools (browser_use_desktop)
- ‚ú?MUST run auto-install script before any browser automation
- ‚ú?Only proceed with browser automation AFTER successful installation
- ‚ú?Script handles all dependency checks automatically
...
1. Default toolchain not configured ‚Ü?Run `rustup default stable` with full path
2. PATH not updated ‚Ü?Use full paths to cargo/rustup, or manually add `~/.cargo/bin` to PATH
...
# Get elements
AGENT_BROWSER_HOME=$AB_HOME ./bin/agent-browser --cdp 9222 snapshot -i
# Output: textbox "ÊêúÁ¥¢Â∞èÁ∫¢‰π? [ref=e2]
