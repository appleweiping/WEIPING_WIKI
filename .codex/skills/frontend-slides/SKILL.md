...
description: Create stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files. Use when the user wants to build a presentation, convert a PPT/PPTX to web, or create slides for a talk/pitch. Uses the local beautiful-html-templates library for template-backed deck generation and helps non-designers discover their aesthetic through visual previews.
---
...
Use this local template library whenever it is available:

`D:\Research\vipin's knowledgebase\skill\llm4ppt(html)\beautiful-html-templates`

This directory is the default visual template source for this skill. For new presentations and PPT conversions, the normal path is:

1. Read `AGENTS.md` in the template library once at the start of the deck-building task.
2. Read `index.json` and shortlist three templates by matching the user's occasion, mood, tone, formality, density, and light/dark preference.
3. Build three title-slide previews from the actual template folders, using the user's real topic/title/subtitle instead of placeholder text.
4. Open the previews and ask the user which template feels right.
5. Clone the chosen template folder into the user's project/workspace and replace placeholder content with the user's real content.
6. If the deck needs a slide layout not present in the chosen template, extend that one template's design system. Do not mix slides from other templates.

Only fall back to the built-in style preset flow when the local template library is missing, unreadable, or the user explicitly asks for a fully custom deck without templates.

When using a template, preserve its fonts, palette, layout grid, decorative vocabulary, slide classes, and navigation runtime. Replace only the authored placeholder content: headings, body text, numbers, captions, names, dates, section labels, and image placeholders.
...
If `D:\Research\vipin's knowledgebase\skill\llm4ppt(html)\beautiful-html-templates` exists, use the template library for style discovery:

1. Ask for occasion and mood/vibe if the user has not already supplied them.
2. Read the template library's `AGENTS.md` and `index.json`.
3. Pick three visually distinct candidate templates whose `mood`, `tone`, `best_for`, `formality`, `density`, and `scheme` match the brief.
4. For each candidate, read its `templates/<slug>/template.html` and generate a real title-slide preview populated with the user's title/topic, subtitle, author/date when available, and logo/images when appropriate.
5. Save previews under `.claude-design/slide-previews/` or another local temp preview folder in the active project.
6. Open all three previews and ask the user to choose one. The selected template is the style source for Phase 3.

Do not use `STYLE_PRESETS.md` while the template library path is available unless the user explicitly asks for a custom non-template style.

### Built-in fallback style path

Use this fallback only when the local template library is unavailable or the user explicitly asks for non-template custom design.
...
If Phase 2 selected a `beautiful-html-templates` template:

1. Clone/copy the chosen `templates/<slug>/` folder into the user's working output folder.
2. Keep the template's CSS, font imports, color variables, layout grid, slide classes, decorative elements, and runtime files intact.
3. Replace placeholder content with the user's real content across the deck.
4. Duplicate existing slide layouts when more slides are needed; remove unneeded demo slides from the bottom when fewer slides are needed.
5. Update slide numbers, table-of-contents labels, section labels, captions, dates, names, stats, and image placeholders.
6. For missing slide types, design new slides using only the chosen template's visual system. Do not mix in another template.
7. Preserve viewport fitting: no slide scroll, no overflow, and content split across slides when density is too high.
8. Open the final HTML deck in the browser and return the absolute file path.

When using a template, the generated output may be a template folder with `template.html` plus sibling assets rather than a single standalone HTML file. Keep sibling assets if the template depends on them.

### Built-in fallback generation

Use this fallback only when Phase 2 used the built-in preset path.
...
| -------------------------------------------------- | -------------------------------------------------------------------- | ------------------------- |
| `D:\Research\vipin's knowledgebase\skill\llm4ppt(html)\beautiful-html-templates\AGENTS.md` | Template library operating manual: shortlist templates, make previews, adapt chosen template | Phase 2 and Phase 3 default path |
| `D:\Research\vipin's knowledgebase\skill\llm4ppt(html)\beautiful-html-templates\index.json` | Catalog of available templates with mood, tone, occasion, density, and scheme metadata | Phase 2 default path |
| `D:\Research\vipin's knowledgebase\skill\llm4ppt(html)\beautiful-html-templates\templates\` | Source folders for clone-and-adapt template generation | Phase 3 default path |
