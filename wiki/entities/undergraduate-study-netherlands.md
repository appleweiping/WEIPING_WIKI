---
title: Undergraduate Study Netherlands
type: entity
status: active
created: 2026-05-10
updated: 2026-05-10
tags:
  - entity
  - study-archive
  - undergraduate
  - netherlands
source_pages:
  - 2026-05-10-d-drive-portfolio-and-undergraduate-roots
source_files:
  - D:/Undergraduate_study_netherlands
---

# Undergraduate Study Netherlands

## Summary

`D:/Undergraduate_study_netherlands` is Vipin's broad undergraduate study archive. It contains course materials, notes, attachments, recordings/media folders, and study resources across CS, EE, applied physics, and languages.

## Key Facts

- EXTRACTED: The requested lowercase/hyphen path was absent; the discovered current path is `D:/Undergraduate_study_netherlands`.
- EXTRACTED: The root is a git repository with remote `https://github.com/appleweiping/Undergraduate-Study-Notes-Netherlands-.git`.
- EXTRACTED: Top-level domains include applied physics, computer science and engineering, electrical engineering, languages, notes, textbooks, and TU/e learning folders.
- EXTRACTED: Course-code anchors include examples such as `2IRR40`, `5EID0`, `31EMA`, and many others.
- EXTRACTED: Current git status shows many deleted tracked course files and many untracked files, so this repo needs privacy and git-state triage before any push.

## Public / Private Boundary

- Public-safe: course-code index, broad folder map, and user-owned study notes when explicitly selected.
- Sensitive or cautious: personal information spreadsheets, consent forms, images, recordings, Office Lens scans, copyrighted course PDFs, and any identity or administrative documents.

## Dynamic Discovery

- If exact path lookup fails, search for `Undergraduate_study_netherlands`, `study`, `netherlands`, and course-code folder patterns.
- Use course-code folders as anchors, but inspect live contents because folders and course materials may move.
- Do not assume tracked git content is safe to publish; deleted/untracked status may indicate local reorganization or privacy cleanup.

## Related

- [[local-project-roots]]
- [[2026-05-10-d-drive-portfolio-and-undergraduate-roots]]
- [[2026-04-22-security-course-self-study-guide]]
