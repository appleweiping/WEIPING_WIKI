...
The shared workstation skill owns dry-run C:/D:/G: inventory manifests, risk classification, `D:/Research` resolved-path exclusion, type-grouped and age-gated user-approved file move batches, D-drive root move-with-junction organization, broad-approval execution, approval packets, rollback manifests, and devtools/agent-resources/vipinknowledge skill-link sync checks.
