...
| `D:/DELVTOOLS_PUBLIC` / `D:/devtools-public` | Public-safe export of selected launchers, examples, docs, and safety checks. `D:/DELVTOOLS_PUBLIC` is the current local checkout; `devtools-public` is the GitHub and historical local name. | Public repo. Run safety/history/pre-push gates before tag, commit, or push. |
...
| `D:/AGENTIC_SCIENCE` | Public agentic-science method repo. Currently contains UUPF, the Universal Upgrade Forge 108-pass audit/upgrade engine. | Treat UUPF as audit/planning evidence. Keep generated reports in the target repo's ignored work area unless deliberately released. |
...
| `D:/Research/WEIPING_LAB` / `D:/Research/vipin-lab` | Honest research workbench for project artifacts, evidence packets, validations, and research-state operations. | Route through `AGENTS.md`, README, and skill before claims or edits. Do not make wiki maintenance depend on its private runtime state. |
| `D:/Research/WEIPING_COUNCIL` / `D:/Research/vipin-council` | Multi-model deliberation runtime and companion review layer for research/lab artifacts. | Route through `AGENTS.md`, README, and skill. Keep handoffs artifact-based and avoid hidden runtime coupling. |
