---
title: Research Projects
type: topic
status: active
created: 2026-05-10
updated: 2026-05-10
tags:
  - topic
  - research
source_pages:
  - 2026-05-10-research-project-roots
---

# Research Projects

## Scope

This topic tracks Vipin's active research projects and their durable claim boundaries.

## Project Clusters

- AI4EDA / analog design agents: [[analog-agent]]
- LLM-based recommendation uncertainty: [[uncertainty]], [[truce-rec]]
- Temporal recommendation evidence: [[tgl-rec]]
- Agent benchmark / specification grounding: [[donebench]]
- AI4S protein uncertainty: [[uncertaintyprotein-ai4s]], [[protein-optimization-feedback-shift]]

## Cross-Cutting Themes

- EXTRACTED: Several projects use strict evidence labels and forbid paper claims before audits/provenance pass.
- EXTRACTED: Recommendation projects converge on same-candidate score schemas and official-baseline governance.
- EXTRACTED: AI4EDA and AI4S projects both study closed-loop decision systems where uncertainty must remain useful under distribution shift or expensive truth queries.
- INFERRED: Vipin's research portfolio is increasingly about decision-relevant uncertainty, not uncertainty as a passive prediction metric.
- INFERRED: For future科研想法, the wiki should support aggressive reframing and [[research-ideation-policy]], not only summarizing the current project state.

## Counterpoints and Gaps

- The current wiki pass is a high-level project digest, not a full ingest of every draft, PDF, result table, or server output.
- Many claims remain status-bound to each project's own docs; final paper claims require deeper per-project evidence checks.
- Public wiki pages should avoid copying sensitive raw outputs, credentials, private server paths, or large artifact details.

## Related

- [[vipin-research-project-map]]
- [[research-ideation-policy]]
- [[llm-based-recommendation]]
- [[personal-knowledge-systems]]
