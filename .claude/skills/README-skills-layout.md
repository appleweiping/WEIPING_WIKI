...
For whole-computer maintenance, project routing, or organization tasks, read `wiki/concepts/whole-computer-project-map.md` first. For D-drive infrastructure detail, also read `wiki/concepts/d-drive-project-map.md`, which separates public exports, private runtime state, skills/resources, and research isolation. For physical C:/D:/G: file moves, use `D:\agent-resources\skills\vipin\workstation-maintenance` before trusting any manifest or type-grouped age-gated move plan, and run its full-plan or exact-batch non-moving preflight before approval. For immediate D-root directory organization, use the skill's drive-root plan/invoke scripts to move eligible roots under `D:\_Organized` while preserving old paths as junctions. Broad user approval can cover all currently passing low-risk batches.
...
## WEIPING_WIKI Maintenance Skill

Claude Code and OpenCode can trigger the project maintenance skill from `D:\Research\vipin's knowledgebase\.claude\skills\vipin-wiki\SKILL.md`; the physical folder is retained as the historical `vipin-wiki` alias while the skill name is `weiping-wiki`.
...
Before relying on Opus, Sonnet, or Haiku through Claude Code, run from the WEIPING_WIKI root when available:
